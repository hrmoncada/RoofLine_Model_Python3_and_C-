...
'''
The purpose of the shebang line (#!/usr/bin/python3) on the top of the script is to define the location of the interprete
...
$ python3 Extract_data_and_build_roofline_model_2.py
...
OUTPUTS :
    GPU_Crusher_Roofline_Model_LOG.pdf
    GPU_Crusher_Roofline_Model.pdf
'''
# Import modules
import csv
import re
...
# Split :  get rid of all the duplicate whitespaces and newline characters, then you can use,
# join() function with string split() function to join with comma ", " and empty space " ".
# The "with" statement creates a context manager and it will automatically close the file
# handler for you when you are done with it. "open()" open a file, read all the lines out of it,
# "as data",  Use "data" to refer to the "data" objec
...
# when the pattern is found
           if re.search(pattern, line):
              if pattern=="invocations" :
                 line = " ".join(line.split()) # split line and join with a " " empty space character as separator
                 #print(line_count," ",line,"\n")
              else :
                 line = ",".join(line.split()) # split line and join with a "," comma character as separator
                 #print(line_count," ",line,"\n")

              line_count += 1
              Store.append(line) # Insert "line" into the end of an existing list object "Store".
...
def extract_metrics(filename, patterns):
    ...
    for i in range(0,len(patterns)):
       Data = []
       [num_rows, num_cols] = search_pattern_and_store_in_data(filename, patterns[i], Data)
       ...
       for j in range(0,len(Data)):
          ...
          #print(Data[i][18], Data[i][19])
       #print("Bytes move  %s = (%f, %f)" %(patterns[i],FETCH_SIZE, WRITE_SIZE))
...
# Initialize 2D array "Average" with zeros, and size (num_rows, num_cols) = (0:19,0:3)
      num_rows = 20
      num_cols = len(patterns)
      Average = [[0 for cols in range(num_cols)] for rows in range(num_rows)]

# For-Loop over label patterns (dadd, dfma, dmul)
      for i in range(0,len(patterns)):
         Pattern_Name = kernelname[k] + '.' + patterns[i]

# Declare 2D string array to store
...
# Kernels
    kernelname = ["0_0_0_0","1_1_0_0","1_1_1_0"] # kernel label "1_0_0_0" add after
...
print("Aritmetic_Intensity = ")
print(*Aritmetic_Intensity, sep = '\n')
...
print("\nAttainable Peak Performance = ")
print(*Attainable_Peak_performance, sep = '\n')
